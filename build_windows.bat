...
if "%GITHUB_ACTIONS%"=="true" (
  mkdir build\custom-triplets || goto :error
  copy "%VCPKG_INSTALLATION_ROOT%\triplets\%triplet%.cmake" "build\custom-triplets\%triplet%.cmake" || goto :error
  echo set(VCPKG_BUILD_TYPE release^) >> build\custom-triplets\%triplet%.cmake || goto :error
  set options=%options% -D VCPKG_OVERLAY_TRIPLETS=%cd%/build/custom-triplets
)
